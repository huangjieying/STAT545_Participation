...
```{r}
(a <- 7:-2)  ## add the bracket will print out a
(n <- length(a))
```
...
```{r}
a*1:3  ## R will recycle through the shorter length. It will show warning because the shorter vector is chopped. If we do a*1:2, it won't show warning, because the longer length can be divided by 2
```
...
```{r}
a>0 & a<5
a>0 && a<5 ## this time it only check the first statement of both left and right handsides
```
...
course  ## this changes the course object
names(course)
```
...
```{r}
course<-c(subject="STAT", code="545A", section="501") ## dont need to make subject/code/section a string
course
```
...
```{r}
my_list[1]
my_list[[1]] ## just give the content of the string, not including the name
my_list[[2]][1]
```
...
```{r}
as.list(small_df)
## Matrix: this require all data column must be in the same type! Not ideal for storing data
diag(5)
```
