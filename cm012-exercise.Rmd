...
```
Difference between factor and character factor, they looks like the same, but factor is actually integar, has levels
```{r}
...
```{r}
set.seed(10) ##
(draw <- sample(letters[1:3], size = 10, replace = TRUE)) ## sample function return random samples
```
...
```{r}
rbind(lotr1, lotr2) %>% str() ## lotr1 and lotr2 have different levels/factors so rbind() makes assumption that what we want is adding a level
bind_rows(lotr1, lotr2) %>% str() ## bind_rows doesn't make assumption, it will still do the same thing but gives warning 
as.character(5) ## coercing means chaning something to others
```
...
gap_small  %>% 
  ...
  geom_bar() +
  scale_x_discrete(drop=FALSE) ## keep all factors
```
...
```{r, fig.width=2, fig.height=1}
gapminder %>% 
  mutate(continent=fct_reorder(continent, lifeExp)) %>% ## median is the default
...
gap_asia_2007$country %>% 
  fct_relevel("Syria","Sweden", after=2) %>% ## after=2, put syria on the third ; can relevel more than one as well, just add country after
  levels() %>% 
  head()
```
...
```{r}
cont %>% 
  fct_lump() %>% ##lump the least frequent levels together as "Other"
  qplot()
```
...
```{r, fig.width=2, fig.height=1}
cont %>% 
  fct_lump(n=2) %>% ##lump the others bar that together would be greater than first 2 bars? 
  qplot()
```
