...
```{r}
if_else(letters =="h", "H", letters ) ## letters contain 26 letters in r

gapminder %>% 
  mutate(lifeExp= if_else(country=="Canada" & year== 1952, 70, lifeExp)) %>% ##select the row you want to change
...
gapminder %>% 
  ...
  summarise(mu=mean(lifeExp), md=median(lifeExp))
gapminder %>% 
  group_by(continent, country) #doesn't change the dataframe, just add one layer on top of the dataset.
```
...
- No, because pop & gdp still have data not summarized..
...
gapminder %>% 
  ...
  summarize(min = min(min)) %>% ## summary for each continent?
  arrange(min) ## arrange the sharpest change
...
my_df= gapminder %>% 
  mutate(cda_gdp=rep(cda, nlevels(country)), ## each country needs to have same year
         gdp =gdpPercap*pop,
         rel_gdp=gdp/cda_gdp) 
